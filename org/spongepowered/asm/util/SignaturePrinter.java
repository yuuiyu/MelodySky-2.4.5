...
public class SignaturePrinter
{
    ...
    public String toDescriptor() {
        final StringBuilder args = this.appendArgs(new StringBuilder(), true);
        return args.append(getTypeName(this.returnType, false, this.fullyQualified)).toString();
    }
    
    private StringBuilder appendArgs(final StringBuilder sb, final boolean typesOnly) {
        sb.append('(');
    ...
    public static String getTypeName(final Type type, final boolean box, final boolean fullyQualified) {
        switch (type.getSort()) {
            ...
            case 9: {
                return getTypeName(type.getElementType(), box, fullyQualified) + "[]";
            }
            case 10: {
