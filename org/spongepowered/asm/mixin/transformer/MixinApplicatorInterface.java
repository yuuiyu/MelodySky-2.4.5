    ...
    protected void prepareInjections(final MixinTargetContext mixin) {
        for (final MethodNode method : this.targetClass.methods) {
            try {
                ...
                if (injectInfo != null) {
                    throw new InvalidInterfaceMixinException((IReferenceMapperContext)mixin, injectInfo + " is not supported on interface mixin method " + method.name);
                }
                continue;
            }
            catch (InvalidInjectionException ex) {
                final String description = (ex.getInjectionInfo() != null) ? ex.getInjectionInfo().toString() : "Injection";
                throw new InvalidInterfaceMixinException((IReferenceMapperContext)mixin, description + " is not supported in interface mixin");
            }
        }
    }
